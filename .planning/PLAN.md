...
- **No paid services.** Allowed: adsb.lol public API (no key today), Cesium ion **Community** token (free, non-commercial), Re:Earth terrain (keyless), EOX Sentinel-2 cloudless (keyless), NASA GIBS VIIRS Black Marble night lights (keyless; acknowledgment in the credits; added by WP-E2), OurAirports (public domain), Cloudflare Tunnel / Tailscale free tiers.
...
### 5.4 Terrain & sun (user feedback #3, 2026-09-22) — WP-E0 contract, then E1–E5 in parallel, then E-A

Chase mode gets visible topography and sunlight. Design, decisions (D1–D13) and the proof of concept: [`terrain-sun-design.md`](terrain-sun-design.md); PoC page, CDP bench and screenshots: `plans/assets/WP-E/poc/`. In short: terrain requests vertex normals, a `Topography` object animates `scene.verticalExaggeration` between 1 + 1e-5 and 0 (mountains sink into the flat satellite map and grow back), and an app-owned sun light follows the render time (a replay is lit at its recorded time). Night keeps the mountains faintly visible and fades in NASA's city lights. **No cast shadows** (measured cost, acne on the aircraft, small visual gain; the user decided). Lighting applies in chase only; browse stays the unlit street map.

| WP | Needs | Owns | Produces |
|---|---|---|---|
| **E0** Contract — `plans/WP-E0-terrain-sun-contract.md` | B-A | `client/scene/exaggeration.ts` + test; additive edit to `client/types.ts` | `TOPO_ON`, `drawnHeightM`, `trueHeightM`, `rescaleSampledM`, `smoothstep`; `ScenePrefs`, `TerrainFrame` |
| **E1** Topography | E0 | `client/scene/topography.ts` + test, `harness/topography.*`; edits `client/scene/terrain.ts`, `client/config.test.ts`, `client/scene/model.ts` + test, `client/scene/chaseCamera.ts` + test | `class Topography` (grow/sink animation, picker nudge, relH latch, lag-corrected ground), `pickRelHM`, `ChaseCameraOpts.groundAt`, vertex normals with their own cache key, the chase model not exaggerated |
| **E2** Sun | E0 | `client/scene/sun.ts` + test, `client/scene/nightLights.ts` + test, `harness/sun.*` | sun ephemeris helpers, `sunLook`, `aimLight`, `parseSunParam` (`?sun=`), `sunTimeMs`, `class Sun`, `makeNightLayer` |
| **E3** Ground objects | E0 | edits `client/scene/runways.ts` + test, `client/scene/fleetLayer.ts` + test | runways per airport, lit, shifted with the terrain; ground icons follow the drawn terrain |
| **E4** Scene toggles | E0 | `client/ui/scenePrefs.ts`, `client/ui/sceneToggles.ts` + tests, `client/ui/sceneToggles.css`, `harness/scene-toggles.*` | `readScenePrefs`, `writeScenePrefs`, `mountSceneToggles` ("3-D terrain", "Sun") |
| **E5** Recorded sun time | base | additive edit to `shared/api.ts`; edits `server/poller.ts` + test, `server/main.test.ts` | `StatusBrief.upstreamOffsetMs?` (server − upstream clock), so replays are lit at their recorded time |
| **E-A** Integration | E1–E5 | edits `client/app.ts` + test, `client/ui/layout.css`; `plans/assets/WP-E-A/gen-synthetic-lowi.ts` | frame order, toggles and keys T/L, preferences, chase-only lighting, night-lights credit, gate GE |

**Plan validation (2026-09-23):** every E package was implemented and tested in a scratch copy of the integrated tree before its plan was written; each plan was then adversarially reviewed and fixed. An independent assembler rebuilt the code from the seven plans alone on a fresh export of `main`: `tsc` clean, **680/680 tests**, `vite build` ✓, byte-identical to the validated tree. Gate GE was run in headless Chrome on the M2 against that tree (WP-E-A Task 5): FPS p50 137 / p5 103, no long task during 10 terrain toggles, worst animation frame 45 ms, 0 empty terrain readings after the animations, camera clearance ≥ 326 m. Screenshots: `plans/assets/WP-E-A/gate-GE/`.

**Gate GE (after E-A):** on your Mac, the synthetic LOWI replay chased over the Nordkette with topography and sun on: FPS p50 ≥ 60, p5 ≥ 30 after shader warmup; worst frame of each terrain animation reported (no long task from the toggles); camera ≥ 15 m above the drawn terrain on every frame; 0 empty terrain readings outside the animation windows; screenshots of morning, golden hour and night at LOWI, KSFO day, the grow sequence, and browse at night (street map not darkened). Steps: WP-E-A Task 5.
