...
// browse centre and height, or the chased aircraft's position), the chased aircraft (?hex=), the chase orbit
...
export function writeUrl(search: string, s: UrlState): string {
  const q = new URLSearchParams(search)
  for (const k of ['at', 'hex', 'cam', 'topo', 'light', 'glass']) q.delete(k)
  if (s.at) q.set('at', `${s.at.lat.toFixed(4)},${s.at.lon.toFixed(4)},${Number(s.at.heightKm.toPrecision(3))}`)
  if (s.hex) q.set('hex', s.hex)
  if (s.hex && s.cam) q.set('cam', `${Math.round(s.cam.headingDeg)},${Math.round(s.cam.pitchDeg)},${Math.round(s.cam.rangeM)}`)
